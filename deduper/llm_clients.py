...
SYSTEM_PROMPT = """你是资深重构工程师。你将收到若干 CPD 重复代码组的结构化信息（仅路径与行号）。
...
3) 模型重点负责给出文件与行号区间；common_code/replacement 允许留空字符串，系统会按固定规则回填。
4) common_file 使用项目内相对路径。
5) 尽量减少行为改变，优先提取公共函数。
6) 输入不包含源码正文，只包含文件路径、总行数与重复区间坐标，请根据坐标准确输出 start_line 和 end_line。
"""
...
def _build_user_prompt(groups: list[DuplicationGroup], files_context: dict[str, str]) -> str:
    ...
    payload = {
        ...
        "requirements": {
            ...
            "line_number_info": "只基于文件路径、总行数和 occurrence 的 start_line/end_line 输出坐标",
        },
    }
